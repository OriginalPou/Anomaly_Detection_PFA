...
#ifndef __AI_ANOMALY_DETECTION_H__
#define __AI_ANOMALY_DETECTION_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_ANOMALY_DETECTION_MODEL_NAME          "anomaly_detection"

#define AI_ANOMALY_DETECTION_ACTIVATIONS_ALIGNMENT   (4)

#define AI_ANOMALY_DETECTION_IN_NUM       (1)
#define AI_ANOMALY_DETECTION_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 270, 1, NULL), \
}
#define AI_ANOMALY_DETECTION_IN_SIZE { \
  (1 * 1 * 270), \
}
#define AI_ANOMALY_DETECTION_IN_1_SIZE  (1 * 1 * 270)
#define AI_ANOMALY_DETECTION_IN_1_SIZE_BYTES  ((1 * 1 * 270) * 4)





#define AI_ANOMALY_DETECTION_OUT_NUM      (1)
#define AI_ANOMALY_DETECTION_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_ANOMALY_DETECTION_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_ANOMALY_DETECTION_OUT_1_SIZE  (1 * 1 * 1)
#define AI_ANOMALY_DETECTION_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 4)

#define AI_ANOMALY_DETECTION_N_NODES (9)

AI_API_DECLARE_BEGIN

/*!
 * @defgroup anomaly_detection
 ...
 * @ref ai_anomaly_detection_create. Both the activations memory buffer 
 ...
 * using @ref ai_anomaly_detection_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_ANOMALY_DETECTION_*
 * defines into file @ref anomaly_detection.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_anomaly_detection_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_ANOMALY_DETECTION_H__*/
